...
BOOL OSXStack::deviceInquiryLock(JNIEnv* env) {
    if (deviceInquiryInProcess && deviceInquiryTerminated) {
        ...
        dispatch_semaphore_wait(deviceInquiryFinishedEvent, kDurationMillisecond * 1000 * 3);
    }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    // wait until the OS X thread has initialized before returning
        OSStatus status = dispatch_semaphore_wait(threadParams.initializedNotificationEvent, kDurationMillisecond * 1000 * 10);
