...
-(BOOL) wait {
    ...
    return (kMPTimeoutErr == dispatch_semaphore_wait(*_notificationEvent, kDurationMillisecond * 1000));
}
...
JNIEXPORT jstring JNICALL Java_com_intel_bluetooth_BluetoothStackOSX_getRemoteDeviceFriendlyName
  (JNIEnv *env, jobject, jlong address) {
    ...
    while ((stack != NULL) && (stack->deviceInquiryBusy)) {
                dispatch_semaphore_wait(stack->deviceInquiryBusyEvent, kDurationMillisecond * 500);
    }
    ...
    while ((stack != NULL) && (runnable.error == 0) && (!runnable.bData)) {
                dispatch_semaphore_wait(runnable.inquiryFinishedEvent, kDurationMillisecond * 500);
    }
    //if (stack != NULL) {
    //  stack->deviceInquiryUnlock();
    //}
